...
public class KkuSystem implements FileNames, ReserveTasks, OptionalReserveTasks {
    ...
    public void showCLIMenu() {
        ...
        while (true) {
            ...
            System.out.println("10. Find all reservations by user");
            ...
            switch (s) {
                case "1":
                    ...
                    int age = sc.nextInt();
                    if (age <= 0) {
                        System.out.println("Age must greater than zero ");
                        break;
                    }
                    ...
                    t = sc.nextInt();
                    ReserveBlock reserveBlock = new ReserveBlock(p, l, t, r);
                    st = this.reserveBlock(reserveBlock);
                    break;
                case "2":
                ...
                case "6":
                    ...
                    System.out.println("At what Clock 1-24 (Only 1 hour can be booked)?");
                    t = sc.nextInt();
                    reserveBlock = new ReserveBlock(p, l, t, r);
                    st = this.removeBlock(reserveBlock);
                    break;
                case "7":
                    System.out.println("Enter person name, id, age (press Enter after each):");
                ...
                case "10":
                    ...
                    String uID = sc.next();
                    Person person = people.getPersonById(uID);
                    this.printReservedBlocks(this.findReservedBlocksBy(person));
                    break;
                default:
    ...
    @Override
    /**
     * Check person is not blocked Check block's date, time, & room is not in
    ...
    public List<ReserveBlock> findReservedBlocksBy(Person person) {
        ...
        for (ReserveBlock block : reservations) {
            if (block.getBy().getId().equalsIgnoreCase(person.getId())) {
                System.out.println(block.getBy());
                result.add(block);
            }
        }
        return result;
    ...
    @Override
    public List<Person> findPeopleReservedBlock(ReserveBlock block) {
        List<Person> result = new ArrayList<>();
        return result;
    }

    @Override
    public List<Integer> findHoursForReservedBlock(ReserveBlock block, LocalDate date) {
        List<Integer> result = new ArrayList<>();
        for (ReserveBlock reservedBlock : reservations) {
            if (reservedBlock.getRoom().equals(block.getRoom()) && reservedBlock.getDate().equals(date)) {
                result.add(reservedBlock.getClock());
            }
        }
        return result;
    }
    ...
    @Override
    public List<LocalDate> findDatesForReservedBlock(ReserveBlock block) {
        List<LocalDate> result = new ArrayList<>();
        for (ReserveBlock reservedBlock : reservations) {
            if (reservedBlock.getRoom().equals(block.getRoom()) && reservedBlock.getClock() == block.getClock()) {
                result.add(reservedBlock.getDate());
            }
        }
        return result;
    }
